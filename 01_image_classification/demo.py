...
def load_7seg_classifier():
    interpreter = None

    print('Load : model/7seg_classifier.tflite')
    interpreter = tf.lite.Interpreter(
        model_path='model/7seg_classifier.tflite')
...
def preprocess_binarization(image, inverse_flag, binarize_th):
    ...
    if binarize_th is None:
        _, temp_image = cv.threshold(temp_image, 0, 255,
                                     cv.THRESH_BINARY + cv.THRESH_OTSU)
    else:
        _, temp_image = cv.threshold(temp_image, binarize_th, 255,
                                     cv.THRESH_BINARY)
    ...
    if extract_image is not None:
        for index in range(num_digits):
            ...
            if most_common_number == 10:
                ...
                most_common_number_str = ' '
            cv.putText(extract_image, most_common_number_str,
                       (temp_x + 10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.8,
...
def main():
    ...
    # tfliteモデルロード
    interpreter = load_7seg_classifier()
    ...
    while True:
        ...
        if extract_image is not None:
            ...
            for index in range(num_digits):
                # 識別対象画像を切り抜き
                temp_start_point = int(
                    (extract_image.shape[1] / num_digits) * index)
